      ...
      <main className="min-h-screen bg-[#F4F4F5] text-zinc-900 font-sans print:bg-white flex flex-col w-full overflow-x-hidden box-border">
        ...
        <header className="h-10 md:h-16 bg-white border-b border-zinc-200 flex items-center justify-between px-2 md:px-8 sticky top-0 z-20 shrink-0 print:hidden shadow-sm w-full box-border">
          <div className="flex items-center gap-2 md:gap-4 min-w-0">
            ...
            >
              <span className="scale-75 md:scale-100 flex items-center"><HugeiconsIcon icon={ArrowLeft01Icon} size={18} /></span>
            </Link>
          ...
          >
            <span className="scale-75 md:scale-100 flex items-center shrink-0"><HugeiconsIcon icon={PrinterIcon} size={14} /></span>
...
function AuditRow({ label, value, isMono = false }: { label: string, value: React.ReactNode, isMono?: boolean }) {
  return (
    <div className="flex flex-col border-b border-zinc-100 pb-2 md:pb-3 w-full box-border min-w-0">
      ...
      <span className={`text-[9px] md:text-[13px] font-medium text-zinc-900 truncate ${isMono ? 'font-mono tracking-tight' : ''}`}>
        {value}
      </span>
    </div>
  )

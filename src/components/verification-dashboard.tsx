...
import { submitIdentityVerification } from "@/actions/verification.action"
import {
  ArrowLeft01Icon,
  ArrowRight01Icon,
  Camera02Icon,
...
// ============================================================================
export function VerificationDashboard({ currentUser }: VerificationDashboardProps) {
  // --- Calculate initial step based on KYC Status ---
  const initialStep = 
    currentUser?.kycStatus === 'Verified' ? 4 : 
    currentUser?.kycStatus === 'Pending' ? 3 : 
    1;
    
  const [step, setStep] = useState(initialStep)
  
  // Form State - Pre-populated with user data if it exists
  const [fullName, setFullName] = useState(currentUser?.legalName || currentUser?.name || "")
  ...
  const [profilePreview, setProfilePreview] = useState<string | null>(currentUser?.profilePicture || null)
        ...
        <div className="w-full lg:w-72 flex flex-col gap-6 shrink-0">
          ...
          <div className="bg-white rounded-xl shadow-sm p-8 flex flex-col items-center text-center">
              ...
              {step !== 4 && (
                <>
                  ...
                  <button 
                    ...
                    className="absolute bottom-0 right-0 w-8 h-8 bg-primary rounded-full border-2 border-white flex items-center justify-center hover:bg-black transition-colors cursor-pointer shadow-sm"
                  >
            ...
            {!hasProfilePicture && step !== 4 && (
              <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-2">* Required</p>
            )}
            {step === 4 && (
            ...
            <h2 className="text-lg font-bold text-slate-900">{fullName || "Your Name"}</h2>
            <p className="text-xs text-slate-500 mt-1">Tenant ID: {currentUser?.id?.slice(-5) || "88492"}</p>
          </div>
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              <div>
                <h1 className="text-xl font-bold text-slate-900 mb-2">
                  {step === 4 ? "Identity Verification Complete" : "Complete Identity Verification"}
                </h1>
                <p className="text-sm text-slate-500">
                  {step === 4 ? "Your identity is secured. You can now access your properties." : "Required to generate your Smart Lock PIN."}
                </p>
              </div>
              {step < 3 && (
                ...
                <React.Fragment key={s}>
                  <div className="flex flex-col items-center relative">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center z-10 transition-colors ${
                      ...
                      {step >= s && <HugeiconsIcon icon={CheckmarkBadge01Icon} size={12} className={step === s ? 'text-primary' : ''} />}
                    </div>
                  </div>
                  {index < 3 && (
            ...
            <div className="flex justify-between w-full max-w-3xl mt-3 text-xs font-bold text-slate-400 uppercase tracking-widest">
              ...
              <span className={`text-center pl-8 ${step >= 3 ? 'text-primary' : ''}`}>Review</span>
                  ...
                  >
                    ...
                    {currentUser?.kycStatus === 'Rejected' && (
                      <div className="mb-6 p-4 bg-red-50 border border-red-100 rounded-lg flex items-start gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase tracking-widest text-red-800 mb-1">Verification Rejected</p>
                          <p className="text-xs font-medium text-red-600 leading-relaxed">
                            Your previous submission was declined. Please ensure your ID number is correct and the photo of you holding your ID is clear and well-lit.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mb-10">
                      <div className="relative">
                        <label className="text-xs font-medium text-slate-500 absolute -top-5 left-0">Full Name</label>
                        <input 
                          type="text" 
                          value={fullName}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-2">
                      ...
                      <div className="relative flex items-end border-b border-slate-200 focus-within:border-primary transition-colors pb-1">
                        <label className="text-xs font-medium text-slate-500 absolute -top-5 left-0">Document Number</label>
                        <select 
                          value={idType}
                        ...
                        <input 
                          type="text" 
                          placeholder={idType === "GHA" ? "GHA-000000000-0" : "1234567890"}
                          value={idNumber}
                ...
                {step === 2 && (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500 mb-8 max-w-lg">
                      Please hold your physical {idType === "GHA" ? "Ghana Card" : "Voter ID"} next to your face. Ensure the card number is visible, well-lit, and your face is clear.
                    </p>
                ...
                {step === 3 && (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500 max-w-sm">
                      Your identity documents are securely being reviewed by our Admin team. This usually takes less than 24 hours.
                    </p>
                  </motion.div>
                )}
                ...
                {step === 4 && (
                  ...
                  >
                    ...
                    <p className="text-sm text-slate-500 max-w-sm mb-8">
                      Your identity documents have been approved. Your account is fully active and you are ready to access your properties.
                    </p>
                    <button 
              ...
              <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex justify-between items-center mt-auto">
                ...
                {step === 1 ? (
                  <button 
                    ...
                    className="px-8 py-3 bg-primary text-white text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-black transition-colors disabled:opacity-50 disabled:bg-slate-300 flex items-center gap-2 "
                  >
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    {!hasProfilePicture && (
                      <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest mt-2">* Profile picture required</p>
                    )}
                  </div>
                )}
              </div>

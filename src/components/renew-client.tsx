...
interface RenewClientProps {
  data: {
    ...
    listing: { title: string; price: number; image: string; propertyType: string; };
  };
}
...
export default function RenewClient({ data }: RenewClientProps) {
  ...
  const paystackConfig = {
    ...
    metadata: {
      ...
      custom_fields: [
        { display_name: "Tenant Name", variable_name: "tenant_name", value: data.user.name },
        { display_name: "Renewal For", variable_name: "property", value: data.listing.title }
      ],
    },
  }
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-black py-12 md:py-24 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-3xl mx-auto mb-10">
        <Link href={`/user/dashboard`} className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-500 hover:text-black transition-colors">
        ...
        <div className="bg-white p-6 md:p-8 rounded-xl border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="flex gap-4 mb-8 pb-8 border-b border-slate-100">
            <div className="relative w-24 h-24 rounded-lg overflow-hidden shrink-0 bg-slate-100">
              <Image src={data.listing.image} alt={data.listing.title} fill className="object-cover" />
            </div>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-lg p-5 mb-8 flex items-center justify-between">
             <div className="flex items-center gap-3">
               <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm">
                 <HugeiconsIcon icon={Calendar01Icon} className="text-primary" size={20} />
               </div>
               <div>
                 <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Current Expiry</p>
                 <p className="font-bold text-slate-900">{data.currentEndDate}</p>
               </div>
             </div>
          </div>
          ...
          >
            ...
            {isProcessing ? "Processing..." : `Pay GHS ${data.rentAmount.toLocaleString()} to Renew`}
          </button>

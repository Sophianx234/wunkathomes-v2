...
export default function ManageTenantsClient({
  ...
  const filteredData = useMemo(() => {
    return data.filter((t) => {
      ...
      // 2. Status Dropdown
      const matchesStatus =
        statusFilter === "all" || t.lease.status === statusFilter;

      // 3. Region Dropdown
      const matchesRegion =
        regionFilter === "all" || t.lease.region === regionFilter;
                ...
                >
                  <TableCell className="py-3">
                    <div className="flex items-center gap-3">
                      <Avatar
                        className={`h-9 w-9 border shadow-sm ${tenant.user.accountStatus !== "Active" ? "opacity-50 grayscale border-zinc-200" : "border-zinc-200"}`}
                      >
                      ...
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-sm font-medium leading-tight ${tenant.user.accountStatus === "Suspended" ? "text-zinc-400 line-through" : "text-zinc-900"}`}
                          >
                            {tenant.user.name}
                          </span>
                          {tenant.user.accountStatus !== "Active" && (
                            <Badge
                              variant="outline"
                              className={`px-1.5 py-0 text-[9px] font-bold uppercase tracking-wider h-4 ${getAccountBadgeStyle(tenant.user.accountStatus)}`}
                            >
                  ...
                  <TableCell className="py-3 align-middle">
                    <Badge
                      variant="outline"
                      className={`px-2 py-0.5 rounded-md font-medium border ${getLeaseBadgeStyle(tenant.lease.status)}`}
                    >
          ...
          {selectedTenant && (
            <>
                ...
                <div className="px-6 pt-10 pb-6 border-b border-zinc-100">
                  {selectedTenant.user.accountStatus !== "Active" && (
                    <div className="mb-4 w-full p-2.5 rounded-lg flex items-center gap-2 text-xs font-medium bg-rose-50/80 text-rose-700 border border-rose-200/60">
                      This account is currently suspended. Portal access is
                      restricted.
                    </div>
                  )}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex gap-4 items-center">
                      <Avatar
                        className={`h-16 w-16 border shadow-sm ring-4 ring-zinc-50 ${selectedTenant.user.accountStatus !== "Active" ? "opacity-50 grayscale border-zinc-200" : "border-zinc-200"}`}
                      >
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mt-1.5">
                          <Badge
                            variant="secondary"
                            className="bg-zinc-100 text-zinc-600 hover:bg-zinc-100 px-2 rounded-md font-medium text-[10px] uppercase tracking-widest"
                          >
                            Tenant
                          </Badge>
                ...
                <div className="px-6 py-6 space-y-8">
                  ...
                  <section className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button
                        variant="link"
                        className="h-auto p-0 text-[11px] font-medium text-zinc-500 hover:text-primary tracking-wide"
                      >
                        View Property{" "}
                        <HugeiconsIcon
                    ...
                    <div className="bg-white border border-zinc-200/80 rounded-xl p-3 flex gap-4 shadow-[0_1px_4px_rgba(0,0,0,0.01)] transition-all hover:shadow-[0_2px_8px_rgba(0,0,0,0.04)] cursor-pointer group">
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        <p className="text-[12px] text-zinc-500 mt-0.5">
                          {selectedTenant.lease.location} ·{" "}
                          <span className="font-medium text-zinc-700">
                            {selectedTenant.lease.unitNumber}
                          </span>
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-5 gap-x-4 pt-2">
                      <div>
                        ...
                        <p className="text-sm font-medium text-zinc-900">
                          {new Date(
                            selectedTenant.lease.startDate,
                      ...
                      <div>
                        ...
                        <p className="text-sm font-medium text-zinc-900">
                          {formatCurrency(selectedTenant.lease.totalRentAmount)}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 bg-zinc-50/80 border border-zinc-200/80 rounded-xl p-4 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-200/50"
                        >
                        ...
                        selectedTenant.transactions.map((tx) => (
                          ...
                          >
                            ...
                            <div className="text-right">
                              ...
                              <span
                                className={`text-[10px] font-bold uppercase tracking-wider ${tx.status === "Success" ? "text-teal-600" : tx.status === "Failed" ? "text-rose-600" : "text-amber-600"}`}
                              >
                                {tx.status}
                              </span>
                            </div>
              ...
              <div className="absolute bottom-0 left-0 w-full bg-white border-t border-zinc-200 p-4 space-y-2 ">
                <Button
                  variant="outline"
                  className="w-full rounded-lg h-10 border-zinc-200 hover:bg-zinc-50 text-zinc-900 font-medium "
                >
                    ...
                    <Button
                      onClick={() =>
                        handleToggleStatus(
                          ...
                          selectedTenant.user.accountStatus,
                        )
                      }
                      disabled={isPending}
                      variant="ghost"

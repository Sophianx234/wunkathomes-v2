...
export default function SearchBar() {
  ...
  return (
    <section className="relative z-10 -mt-16 px-4 sm:px-6 ">
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-1 px-4 py-3 hover:bg-slate-50 rounded-full md:rounded-r-none md:rounded-l-full transition-colors group cursor-pointer border-t md:border-t-0 border-slate-100">
          ...
          <div className="flex flex-col w-full">
            <label className="text-[11px] font-bold text-slate-800 uppercase tracking-wider mb-0.5">
              Property Type
            </label>
            <Select
            ...
            >
              ...
              <SelectContent className="bg-white p-0 w-full min-w-[var(--radix-select-trigger-width)]">
                <div className="divide-y divide-slate-100 flex flex-col w-full">
                  ...
                  <SelectItem
                    value="all"
                    className="cursor-pointer w-full [&>span]:w-full font-medium py-3 rounded-none focus:bg-slate-50"
                  >
                ...
                <div className="divide-y divide-slate-100 flex flex-col w-full">
                  <SelectGroup className="w-full divide-y divide-slate-100 flex flex-col">
                    ...
                    <div className="bg-slate-50 py-1.5 px-2">
                      <SelectLabel className="text-xs uppercase tracking-widest text-slate-500 font-bold m-0 p-0 pl-2">
                        Greater Accra
                      </SelectLabel>
                    </div>
        ...
        <div className="p-2 w-full md:w-auto mt-2 md:mt-0">
          <Button
            type="submit"
            disabled={loading}
            className="w-full md:w-auto bg-black text-primary-foreground hover:bg-black/90 px-8 py-6 rounded-full disabled:opacity-70 flex items-center justify-center gap-2 font-bold text-base shadow-md transition-transform hover:scale-[1.02] active:scale-95"
          >
            {loading ? (
      ...
      {/* 🏠 Search Results Area */}
      <div
        className={`max-w-7xl mx-auto mt-16 min-h-[200px] hidden ${loading && "block"}`}
      >
        {/* Error State */}
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex flex-col items-center justify-center text-red-500 bg-red-50 p-6 rounded-2xl max-w-md mx-auto"
          >

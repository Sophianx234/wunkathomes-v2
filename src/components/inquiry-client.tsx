  ...
  return (
    <div className="space-y-6">
      ...
      <section className="flex flex-col md:flex-row items-center gap-3 bg-white p-1.5 border border-zinc-200/60 shadow-[0_1px_4px_rgba(0,0,0,0.01)] rounded-lg w-full">
        ...
        <div className="flex flex-wrap md:flex-nowrap items-center gap-2 w-full md:w-auto px-2 pb-1 md:pb-0">
          <Select value={statusFilter} onValueChange={(val) => setStatusFilter(val as any)}>
            <SelectTrigger className="w-full md:w-[130px] h-8 border-0 bg-zinc-50 hover:bg-zinc-100 text-[12px] font-medium text-zinc-700 focus:ring-0 rounded-md">
      ...
      <div className="bg-white border border-zinc-200/60 rounded-lg overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.01)]">
        <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-zinc-200/60 bg-zinc-50/30 text-[12px] font-medium text-zinc-500 tracking-tight">
          <div className="col-span-12 md:col-span-3">Sender</div>
          <div className="hidden md:block col-span-2">User Context</div>
        ...
        <div className="divide-y divide-zinc-200/60">
            ...
            >
              ...
              <div className="col-span-12 md:col-span-3 flex items-start gap-3">
                <Avatar className="h-10 w-10 mt-0.5 shrink-0 border border-zinc-200/60">
                  <AvatarImage src={inq.profilePicture || ""} />
                ...
                <div className="flex flex-col min-w-0 flex-1">
                  ...
                  <span className="text-[12px] text-zinc-500 truncate flex items-center gap-1 mt-0.5">
                    {inq.email}
                  </span>
                </div>
              </div>
              ...
              <div className="hidden md:flex col-span-2 items-center">
                {inq.isGuest ? (
                  <Badge variant="outline" className="px-2 py-0.5 border rounded-sm border-zinc-200 bg-zinc-50 text-zinc-600 font-bold tracking-widest uppercase text-[9px] flex items-center gap-1 shadow-none">
                    <HugeiconsIcon icon={UserCircleIcon} size={10} /> Guest
                  </Badge>
                ) : (
                  <Badge variant="outline" className="px-2 py-0.5 border border-zinc-200 rounded-sm bg-black text-white font-bold tracking-widest uppercase text-[9px] flex items-center gap-1 shadow-none">
              ...
              {/* Message */}
              <div className="hidden md:flex col-span-3 text-[13px] text-zinc-600 truncate pr-4">
                {inq.message}
              </div>
              ...
              {/* Date */}
              <div className="hidden md:flex col-span-2 items-center justify-end text-zinc-500 gap-1.5">
          ...
          {filteredInquiries.length === 0 && (
            <div className="py-20 text-center flex flex-col items-center justify-center">
              <HugeiconsIcon icon={MessageMultiple01Icon} size={32} className="text-zinc-300 mb-3" />
              <p className="text-[13px] font-medium text-zinc-500">No inquiries found.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <Dialog open={!!selectedInquiry} onOpenChange={(open) => !open && setSelectedInquiry(null)}>
        <DialogContent className="w-full sm:max-w-xl p-0 bg-white border border-zinc-200 rounded-2xl overflow-hidden font-sans">
          {selectedInquiry && (
            <div className="flex flex-col max-h-[85vh]">
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                  ...
                  <div className="rounded-lg border border-zinc-200/60 overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.01)]">
                    <div className="p-4 bg-zinc-50/50 flex gap-4 border-b border-zinc-200/60">
                      <Avatar className="h-12 w-12 border border-zinc-200/60 shrink-0">
                        <AvatarImage src={selectedInquiry.profilePicture || ""} />
                      ...
                      <div className="flex flex-col justify-center min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {selectedInquiry.isGuest ? (
                            <Badge variant="outline" className="px-1.5 py-0 border-zinc-200 bg-zinc-100 text-zinc-600 font-bold uppercase text-[9px] shadow-none">
                              Guest
                            </Badge>
                          ) : (
                            <Badge variant="outline" className="px-1.5 py-0 border-zinc-200 bg-black text-white font-bold uppercase text-[9px] shadow-none">
                              Tenant
                            </Badge>
                          )}
                        </div>
                    ...
                    <dl className="grid grid-cols-2 gap-x-4 gap-y-4 p-4 text-[13px] bg-white">
                      <div className="col-span-2 sm:col-span-1">
                        ...
                        <dd className="font-medium text-zinc-900 truncate" title={selectedInquiry.email}>{selectedInquiry.email}</dd>
                      </div>
                      {selectedInquiry.phone && (
                        <div className="col-span-2 sm:col-span-1">
                          <dt className="text-zinc-500 mb-1 flex items-center gap-2"><HugeiconsIcon icon={SmartPhone01Icon} size={12} /> Phone</dt>
                          <dd className="font-medium text-zinc-900 font-mono tracking-tight">{selectedInquiry.phone}</dd>
                        </div>
                      )}
                    </dl>
                  </div>
                ...
                {/* 2. Message Body */}
                <div>
                  <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3 flex items-center justify-between">
                    Message Details
                    <span className="text-[10px] font-medium text-zinc-400 normal-case flex items-center gap-1">
                      <HugeiconsIcon icon={Clock01Icon} size={12} /> {new Date(selectedInquiry.createdAt).toLocaleString()}
                    </span>
                  </h3>
                  <div className="bg-zinc-50 p-5 rounded-xl text-[13px] leading-relaxed text-zinc-800 border border-zinc-200 whitespace-pre-wrap min-h-[100px]">
              ...
              {/* CRM ACTION BAR */}
              <div className="p-4 border-t border-zinc-200 bg-white flex flex-col sm:flex-row justify-between items-center gap-4">
                
                {/* Left side actions */}
                <div className="flex w-full sm:w-auto gap-2">
                  <a 
                    href={`mailto:${selectedInquiry.email}?subject=Re: Your Inquiry to WunkatHomes`}
                    className="flex-1 sm:flex-none inline-flex h-9 items-center justify-center rounded-lg bg-zinc-50 px-4 text-[12px] font-semibold text-zinc-900 border border-zinc-200 hover:bg-zinc-100 transition-colors gap-2"
                  >
                    Quick Reply
                  </a>
                ...
                <div className="flex w-full sm:w-auto items-center gap-3">
                  ...
                  <Button 
                    ...
                    disabled={isPending}
                    className="flex-1 sm:flex-none h-9 bg-black hover:bg-zinc-900 text-white font-semibold text-[13px] px-6 rounded-lg transition-colors"
                  >
                    {isPending ? (
                      <>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <AlertDialogContent className="border border-zinc-200 bg-white">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter className="mt-4">
            <AlertDialogCancel disabled={isPending} className="border border-zinc-200 bg-zinc-50 hover:bg-zinc-100 font-medium h-9 text-[13px]">Cancel</AlertDialogCancel>

...
const formatCurrency = (amount: number, currency: string = "GHS") => 
  new Intl.NumberFormat('en-GH', { style: 'currency', currency: currency }).format(amount);
...
const getPurposeBadge = (purpose: string) => {
  switch (purpose) {
    case "Upfront_Rent": return "text-blue-700 bg-blue-50/50 ring-1 ring-blue-200/50";
    case "Rent_Balance": return "text-purple-700 bg-purple-50/50 ring-1 ring-purple-200/50";
    case "Monthly_Renewal": return "text-zinc-700 bg-zinc-100/50 ring-1 ring-zinc-200/60";
    case "Purchase": return "text-emerald-700 bg-emerald-50/50 ring-1 ring-emerald-200/50";
    default: return "text-zinc-700 bg-zinc-100/50 ring-1 ring-zinc-200/60";
  }
};

const getStatusBadge = (status: string) => {
  switch (status) {
    case "Success": return "text-emerald-700 ring-1 ring-emerald-200/60 bg-emerald-50/30";
    case "Pending": return "text-amber-700 ring-1 ring-amber-300/60 bg-amber-50/50 animate-pulse-slow";
    case "Failed": 
    case "Abandoned": return "text-zinc-500 ring-1 ring-zinc-200/60 bg-zinc-50/50 line-through decoration-zinc-300";
    case "Refunded": return "text-rose-700 ring-1 ring-rose-200/60 bg-rose-50/50";
    default: return "text-zinc-500 ring-1 ring-zinc-200/60 bg-zinc-50/50";
  }
};

const getChannelIcon = (channel: string) => {
  switch (channel) {
    case "card": return <HugeiconsIcon icon={CreditCardIcon} size={14} className="text-zinc-500" />;
    case "mobile_money": return <HugeiconsIcon icon={SmartPhone01Icon} size={14} className="text-zinc-500" />;
    case "bank": return <HugeiconsIcon icon={UniversityIcon} size={14} className="text-zinc-500" />;
    default: return <HugeiconsIcon icon={ArrowUpRight01Icon} size={14} className="text-zinc-500" />;
  }
};
...
export default function TransactionsClient({ data }: TransactionsClientProps) {
  ...
  const filteredData = useMemo(() => {
    if (activeTab === "success") return data.filter(tx => tx.status === "Success");
    if (activeTab === "pending") return data.filter(tx => tx.status === "Pending");
    return data;
  }, [data, activeTab]);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] p-6 lg:pb-10 font-sans print:p-0 print:bg-white">
      ...
      <div id="main-table-view" className="max-w-[1400px] mx-auto space-y-6 pt-12 md:pt-16 print:hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-2">
          ...
          <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as any)} className="w-full md:w-auto">
            <TabsList className="h-9 bg-zinc-100/50 border border-zinc-200/60 p-0.5 rounded-lg">
              <TabsTrigger value="all" className="text-[13px] font-medium data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-md px-4">
                All Payments
              </TabsTrigger>
              <TabsTrigger value="success" className="text-[13px] font-medium data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-md px-4">
                Successful
              </TabsTrigger>
              <TabsTrigger value="pending" className="text-[13px] font-medium data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-md px-4">
                Pending
                {pendingCount > 0 && (
                ...
                >
                  <TableCell className="py-3 align-middle">
                    <div className="flex flex-col">
                      <span className="text-[13px] font-mono font-medium text-zinc-900">{tx.reference}</span>
                  ...
                  <TableCell className="py-3 align-middle">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-md overflow-hidden bg-zinc-100 shrink-0 border border-zinc-200 flex items-center justify-center">
                        {tx.propertyImage ? (
                          <img src={tx.propertyImage} alt={tx.propertyTitle} className="w-full h-full object-cover" />
                        ) : (
                          <HugeiconsIcon icon={Home09Icon} size={14} className="text-zinc-400" />
                        )}
                      </div>
                      <span className="text-[13px] font-medium text-zinc-900">{tx.propertyTitle}</span>
                    </div>
                  </TableCell>

                  <TableCell className="py-3 align-middle">
                    <span className={`px-2 py-0.5 rounded-sm text-[10px] uppercase font-bold tracking-widest ${getPurposeBadge(tx.paymentPurpose)}`}>
                  ...
                  <TableCell className="py-3 align-middle">
                    <div className="flex items-center text-[12px] font-medium text-zinc-600 gap-1.5 capitalize">
                      {getChannelIcon(tx.channel)} {tx.channel.replace("_", " ")}
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="py-3 align-middle ">
                    <Badge variant="outline" className={`px-2 py-0 border-0  text-[10px] uppercase tracking-wider rounded-sm font-bold h-5 ${getStatusBadge(tx.status)}`}>
                      {tx.status}
                    </Badge>
                  </TableCell>
                </TableRow>
          ...
          {selectedTx && (
            <>
              ...
              <div className="absolute bottom-0 left-0 w-full bg-white border-t border-zinc-200/80 p-4 shadow-[0_-8px_20px_rgba(0,0,0,0.03)] z-20">
                ...
                >
                  <HugeiconsIcon icon={selectedTx.status === "Success" ? Download01Icon : File02Icon} size={16} />
                  {selectedTx.status === "Success" ? "Download Receipt" : "Receipt Unavailable"}
                </Button>
              </div>
            </>
          )}
      ...
      {selectedTx && <PrintReceipt isPrintView={true} transaction={selectedTx} />}

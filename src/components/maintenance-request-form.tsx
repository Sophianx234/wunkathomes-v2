...
export default function MaintenanceRequestForm({ properties }: MaintenanceFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6 max-w-4xl mx-auto w-full overflow-x-hidden box-border px-2 md:px-0">
      ...
      <div className="bg-white rounded-lg md:rounded-xl border border-slate-200 p-4 md:p-8 w-full box-border">
        ...
        <div className="space-y-1.5 md:space-y-2 max-w-md w-full min-w-0 box-border">
          <Label className="text-[10px] md:text-[13px] font-medium text-slate-700">Affected Property *</Label>
          ...
          {properties.length === 1 ? (
            ...
            <Select value={selectedLeaseId} onValueChange={setSelectedLeaseId} required>
              <SelectTrigger className="h-9 md:h-10 bg-slate-50 focus:ring-zinc-950 border-slate-200 text-[11px] md:text-sm w-full min-w-0 max-w-full box-border m-0">
                <SelectValue placeholder="Select a property" className="truncate" />
              </SelectTrigger>
              <SelectContent>
                {properties.map((property) => (
                  <SelectItem key={property.id} value={property.id} className="text-[11px] md:text-sm">
                    {property.title}
                  </SelectItem>
                ))}
              </SelectContent>
      ...
      <div className="bg-white rounded-lg md:rounded-xl border border-slate-200 p-4 md:p-8 w-full box-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5 w-full box-border">
          <div className="space-y-1.5 md:space-y-2 w-full min-w-0 box-border">
            <Label className="text-[10px] md:text-[13px] font-medium text-slate-700">Service Category *</Label>
            <Select name="category" required>
              <SelectTrigger className="h-9 md:h-10 bg-slate-50 focus:ring-zinc-950 border-slate-200 text-[11px] md:text-sm w-full min-w-0 max-w-full box-border m-0">
                <SelectValue placeholder="Select the affected area" className="truncate" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="Plumbing" className="text-[11px] md:text-sm">Plumbing & Water</SelectItem>
                <SelectItem value="Electrical" className="text-[11px] md:text-sm">Electrical & Lighting</SelectItem>
                <SelectItem value="HVAC" className="text-[11px] md:text-sm">Air Conditioning & HVAC</SelectItem>
                <SelectItem value="Smart_Lock" className="text-[11px] md:text-sm">Smart Lock & Access</SelectItem>
                <SelectItem value="Appliances" className="text-[11px] md:text-sm">Provided Appliances</SelectItem>
                <SelectItem value="Structural" className="text-[11px] md:text-sm">Structural (Doors, Windows, Walls)</SelectItem>
                <SelectItem value="Other" className="text-[11px] md:text-sm">Other</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div className="space-y-1.5 md:space-y-2 w-full min-w-0 box-border">
            <Label className="text-[10px] md:text-[13px] font-medium text-slate-700">Priority Level *</Label>
            <Select name="priority" defaultValue="Routine">
              <SelectTrigger className="h-9 md:h-10 bg-slate-50 focus:ring-zinc-950 border-slate-200 text-[11px] md:text-sm w-full min-w-0 max-w-full box-border m-0">
                <SelectValue placeholder="Assess the urgency" className="truncate" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="Low" className="text-[11px] md:text-sm">Low (No immediate impact on living)</SelectItem>
                <SelectItem value="Routine" className="text-[11px] md:text-sm">Routine (Standard maintenance)</SelectItem>
                <SelectItem value="High" className="text-[11px] md:text-sm">High (Impacting daily comfort)</SelectItem>
                <SelectItem value="Emergency" className="text-[11px] md:text-sm">Emergency (Active leak, security breach, etc.)</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg md:rounded-xl border border-slate-200 p-4 md:p-8 w-full box-border">
        ...
        <div className="space-y-3 md:space-y-5 w-full box-border">
          <div className="space-y-1.5 md:space-y-2 w-full min-w-0 box-border">
            <Label htmlFor="title" className="text-[10px] md:text-[13px] font-medium text-slate-700">Subject *</Label>
            <Input 
              id="title" 
              name="title" 
          ...
          <div className="space-y-1.5 md:space-y-2 w-full min-w-0 box-border">
            <Label htmlFor="description" className="text-[10px] md:text-[13px] font-medium text-slate-700">Detailed Context *</Label>
            <Textarea 
              id="description" 
              name="description" 

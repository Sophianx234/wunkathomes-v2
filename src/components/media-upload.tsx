  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-8">
      <h2 className="text-[18px] font-medium text-slate-900 mb-6 pb-3 border-b border-slate-200 flex items-center gap-2.5">
        <HugeiconsIcon icon={Upload01Icon} size={20} className="text-slate-400" strokeWidth={1.5} />
        Media Upload
      </h2>
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center text-center cursor-pointer transition-colors ${
          isDragActive ? "border-zinc-950 bg-slate-100" : "border-slate-200 bg-slate-50 hover:bg-slate-100/50"
        }`}
      >
        <input {...getInputProps()} />
        <div className="h-12 w-12 rounded-full bg-white flex items-center justify-center mb-4 border border-slate-100">
          <HugeiconsIcon icon={Upload01Icon} size={24} className="text-slate-500" strokeWidth={1.5} />
        </div>
        <p className="text-[14px] font-medium text-slate-900 mb-1">Click or drag images to upload</p>
        <p className="text-[13px] text-slate-500">PNG, JPG, JPEG or WEBP (max. 5MB)</p>
      </div>

...
export default function BookingCard({ listing, isRent }: BookingCardProps) {
  ...
  return (
    <div className="lg:col-span-4  w-[300px] -translate-x-8  relative hidden lg:block">
      ...
      <div className="sticky top-32 min-h-[460px] p-8 border-2 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col rounded-xl">
        ...
        {/* === Simple Info Ledger (Always Visible) === */}
        <div className="space-y-4 mb-8 shrink-0">
          ...
          {isRent && listing.terms?.leaseTerm && (
            <div className="flex justify-between items-center py-3 border-b border-black/10">
              <span className="text-sm font-medium text-slate-600">Minimum Stay</span>
              <span className="text-sm font-bold text-black">{formatLeaseTerm(listing.terms.leaseTerm)}</span>
            </div>
          )}
        </div>
            ...
            <>
              <Link href={`/checkout/${listing.slug}?type=deposit`} className="w-full">
                <button className="w-full py-4 bg-black text-white font-black uppercase tracking-widest text-xs rounded-lg hover:bg-slate-800 transition-colors">
                  {isRent ? 'Reserve Now' : 'Reserve property'}
                </button>
              </Link>
          ...
          {step === 'PHONE' && (
            <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <p className="text-[9px] text-center font-bold uppercase tracking-widest text-slate-400 mt-2">
                We'll send your tour confirmation via WhatsApp.
              </p>
            </div>
          )}
        ...
        {/* === Footer Trust Marker === */}
        {step === 'IDLE' && (
          <p className="text-[10px] text-center text-slate-500 uppercase tracking-widest mt-6 font-bold leading-relaxed shrink-0">
            Managed by WunkatHomes. <br /> Safe and secure payments.
          </p>
        )}
      </div>
    </div>

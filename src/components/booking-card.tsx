...
import { createTourAction, TourActionState } from "@/actions/tour.action";
      ...
      <div className="sticky top-32 min-h-[460px] p-8 border-2 border-black bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col rounded-xl">
        ...
        <div className="mb-8 shrink-0">
          ...
          <div className="text-4xl font-black tracking-tight">
            ...
            {listing.terms?.leaseTerm && (
              <span className="text-lg text-slate-500 font-medium tracking-normal"> {formatLeaseTerm(listing.terms.leaseTerm)}</span>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 mb-8 shrink-0">
          <div className="flex justify-between items-center py-3 border-b border-black/10">
            <span className="text-sm font-medium text-slate-600">Verified Property</span>
            <span className="text-sm font-bold text-black flex items-center gap-1">
              <HugeiconsIcon icon={CheckmarkBadge01Icon} size={16} className="text-green-600" /> Yes
            </span>
          </div>
          <div className="flex justify-between items-center py-3 border-b border-black/10">
            <span className="text-sm font-medium text-slate-600">Agent Fees</span>
        ...
        <div className="flex flex-col gap-3 mt-auto transition-all duration-300">
          ...
          {step === 'IDLE' && (
            <>
              ...
              >
                <HugeiconsIcon icon={Calendar01Icon} size={16} /> Schedule a Tour
              </button>
            </>
          )}
          ...
          {step === 'DATE' && (
            <div className="flex flex-col gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <button onClick={() => handleBack('IDLE')} className="text-[10px] text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1 mb-1 hover:text-black w-fit">
                <HugeiconsIcon icon={ArrowLeft01Icon} size={12} /> Back
              </button>
              
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold uppercase tracking-widest text-black">Select a Date</label>
                <div className="relative">
                  <HugeiconsIcon icon={Calendar01Icon} size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                  <input 
                    type="date" 
                    value={selectedDate}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-bold uppercase tracking-widest text-black">Select a Time</label>
                <div className="relative">
                  <HugeiconsIcon icon={Clock01Icon} size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                  <input 
                    type="time" 
                    value={selectedTime}
          ...
          {step === 'PHONE' && (
            <form action={formAction} className="flex flex-col gap-2 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <button type="button" onClick={() => handleBack('DATE')} className="text-[10px] text-slate-500 uppercase tracking-widest font-bold flex items-center gap-1 mb-2 hover:text-black w-fit">
                <HugeiconsIcon icon={ArrowLeft01Icon} size={12} /> Back
              </button>
              
              <label className="text-xs font-bold uppercase tracking-widest text-black mb-1">Your WhatsApp Number</label>
              <div className="relative">
                <HugeiconsIcon icon={SmartPhone01Icon} size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
              ...
              >
                {isPending && <HugeiconsIcon icon={Loading03Icon} size={16} className="animate-spin mr-2" />}
          ...
          {step === 'SUCCESS' && (
            <div className="flex flex-col items-center justify-center text-center gap-2 py-4 animate-in fade-in zoom-in-95 duration-300">
              ...
              <h3 className="font-black text-lg uppercase tracking-tight text-black">Tour Scheduled!</h3>
              ...
              <button onClick={resetFlow} className="w-full py-3 bg-white text-black font-black uppercase tracking-widest text-xs border-2 border-black rounded-lg hover:bg-slate-50 transition-colors">
                Done
              </button>
            </div>
          )}

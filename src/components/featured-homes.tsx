    ...
    <section id="homes" className="py-16 md:py-24 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-zinc-600 text-lg">Discover handpicked properties available now.</p>
        </div>
...
<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-6 gap-y-10">
  {!loading && homes.map((home, index) => (
    ...
    >
      ...
      <div className="relative aspect-[20/19] overflow-hidden rounded-lg bg-zinc-100/50">
        <img
        ...
        <button className="absolute top-3 right-3 p-2 text-white hover:scale-110 active:scale-95 transition-all z-10 drop-shadow-sm">
        ...
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center gap-1.5 shadow-sm">
          ...
          <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-800">Smart Lock</span>
        </div>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex justify-between items-start gap-2">
          <h3 className="font-semibold text-zinc-900 text-[15px] truncate">
          ...
          <div className="flex items-center gap-1 shrink-0 mt-0.5">
            ...
            <HugeiconsIcon icon={StarIcon} size={14}  className="text-zinc-900" />
            <span className="text-sm font-medium text-zinc-900">{home.rating}</span>
          </div>
        </div>

        {/* Row 2: Property Name/Desc */}
        <p className="text-zinc-500 text-[15px] truncate mt-0.5">
          {home.name}
        </p>

        {/* Row 3: Specs */}
        <p className="text-zinc-500 text-[15px] mt-0.5">
          {home.beds} Beds • {home.baths} Baths
        </p>

        {/* Row 4: Pricing */}
        <div className="mt-2 flex items-baseline gap-1">
          <span className="font-semibold text-zinc-900 text-[15px]">${home.price}</span>
          <span className="text-zinc-900 text-[15px]">/month</span>
        </div>
      </div>
    </motion.div>
  ))}

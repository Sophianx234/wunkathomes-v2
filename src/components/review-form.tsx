...
import { useParams } from "next/navigation"; // To automatically get the property slug
...
export default function ReviewForm({ listingId }: ReviewFormProps) {
  ...
  // React 19 naturally provides 'isPending' as the 3rd element here
  ...
  return (
    <div className="bg-slate-50 p-6 rounded-xl border border-slate-200 mb-8">
      ...
      {/* Hooking formAction directly into the action attribute */}
      <form action={formAction} className="flex flex-col gap-4">
        
        {/* Hidden inputs automatically pass data to your Server Action */}
        ...
        <textarea
          ...
          className="w-full p-4 border border-slate-200 rounded-lg text-sm focus:outline-none focus:border-black focus:ring-1 focus:ring-black resize-y min-h-[100px]"
          maxLength={1000}
        />
        ...
        <button
          ...
          className="self-start px-6 py-3 flex items-center bg-black text-white font-bold uppercase tracking-widest text-[10px] rounded-lg hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >

...
export default function TenantOnboardClient({ tenant }: { tenant: TenantRecord }) {
  ...
  const saveEditedDetails = async () => {
    ...
    try {
      ...
      if (editCardScan) formData.append("idCardPhoto", editCardScan);
  ...
  const needsDocs = tenant.user.kycStatus !== "Verified" && (!tenant.user.ghanaCardUrl || !tenant.user.securityPhotoUrl);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge variant="outline" className="px-2 py-0 border-0 rounded text-[10px] uppercase tracking-wider font-bold h-5 bg-amber-50 text-amber-700 ring-1 ring-amber-200/60">
                    Pending
                  </Badge>
                </div>
        ...
        <div className="bg-white border border-zinc-200/80 shadow rounded-xl p-6 md:p-8">
          <section>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-lg border border-zinc-200/60 bg-zinc-50/50">
                  ...
                  <Input 
                    ...
                    className="h-10 text-[14px] font-mono bg-white"
                    placeholder="GHA-"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-end pt-2 border-t border-zinc-200/60 mt-6">
                <Button 
                  variant="secondary"
                  className="h-11 px-6 text-[14px] font-medium rounded-lg  mt-4"
                  onClick={saveEditedDetails}
                  disabled={isSavingEdit}
                >
                  {isSavingEdit ? "Saving..." : "Save Identity Documents"}
                </Button>
              </div>
            </div>
          </section>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <button
                    ...
                    className="text-[14px] font-medium text-zinc-900 hover:underline underline-offset-4 disabled:no-underline disabled:text-zinc-400"
                  >
                    {tenant.checklist.leaseSigned === "Signed" ? "View Signed Document" : "Awaiting Tenant Signature"}
                  </button>
                </div>
              </div>
              <div>{tenant.checklist.leaseSigned === "Signed" ? <HugeiconsIcon icon={CheckmarkCircle01Icon} size={22} className="text-emerald-600" /> : <HugeiconsIcon icon={Clock01Icon} size={22} className="text-amber-500" />}</div>
            </div>
          </section>
        </div>
        ...
        <div className="bg-white border border-zinc-200/80 shadow rounded-xl p-6 md:p-8">
          <section>
            <div className="mb-6">
              ...
              <p className="text-[14px] text-zinc-500 leading-relaxed">Ensure the tenant is physically present in the office with their original Ghana Card. Verify their identity to activate the lease and provision digital access for <strong className="text-zinc-900 font-semibold">{tenant.lease.propertyName} ({tenant.lease.unitNumber})</strong>.</p>
            </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-[13px] text-zinc-500 leading-relaxed">
              By confirming, you verify that the tenant's physical Ghana Card matches the person present in the office. This action will activate their lease and provision Tuya Smart Lock access codes via SMS.
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          <AlertDialogFooter className="mt-6 gap-2 sm:gap-0">
            ...
            <AlertDialogAction onClick={executeConfirmedAction} disabled={isPending} className="h-10 text-[13px] font-semibold rounded-lg bg-zinc-900 text-white hover:bg-zinc-800">
              {isPending ? <><HugeiconsIcon icon={Loading03Icon} className="animate-spin mr-2" size={14} /> Processing...</> : 
               (tenant.user.kycStatus === "Verified") ? "Approve & Dispatch Keys" : "Verify & Provision Access"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>

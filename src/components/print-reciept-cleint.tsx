...
export default function PrintReceipt({ transaction, isPrintView = false }: PrintReceiptProps) {
  
  const dateToUse = transaction.paidAt || transaction.createdAt;
  
  const formattedDateShort = new Date(dateToUse).toLocaleDateString('en-US', {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
  }).toUpperCase()
  ...
  return (
    <>
      ...
      <div 
        ...
        style={isPrintView ? { WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' } : undefined} 
      >
        <div className="w-full max-w-md px-0 sm:px-4">
          
          <div className={`w-full bg-white border-2 border-slate-200 rounded-t-3xl relative overflow-hidden ${isPrintView ? 'print:break-inside-avoid shadow-none' : 'shadow-sm'}`}>
            
            {/* Top Section */}
            <div className="pt-8 pb-6 px-8 text-center flex flex-col items-center">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 ${transaction.status === 'Success' ? 'bg-green-50 text-green-500' : 'bg-amber-50 text-amber-500'}`}>
                <HugeiconsIcon icon={transaction.status === 'Success' ? TickDouble02Icon : UniversityIcon} size={28} />
              </div>
            ...
            <div className="relative w-full h-8 flex items-center justify-center my-1">
              <div className={`absolute left-0 -ml-4 w-8 h-8 ${isPrintView ? 'bg-white' : 'bg-zinc-100'} border-r-2 border-slate-200 rounded-full`} />
              <div className={`absolute right-0 -mr-4 w-8 h-8 ${isPrintView ? 'bg-white' : 'bg-zinc-100'} border-l-2 border-slate-200 rounded-full`} />
            ...
            <div className="pt-4 pb-8 px-6 sm:px-8">
              <div className="flex justify-between items-start mb-5">
                ...
                <div className="text-right">
                  ...
                  <p className="font-black text-lg text-slate-900">
                    {new Intl.NumberFormat('en-GH', { style: 'currency', currency: transaction.currency }).format(transaction.amount)}
                  </p>
                </div>
              </div>
              ...
              <div className="mb-5">
                ...
                <p className="text-xs font-medium text-slate-500 mt-1">{transaction.propertyLocation}</p>
              </div>
              ...
              <div className="mb-6">
                ...
                <p className="text-xs font-medium text-slate-500 mt-1">{transaction.userEmail}</p>
              </div>
          ...
          {/* Ticket Bottom Teeth */}
          <div className={`flex justify-between w-full px-2 -mt-3 z-10 ${isPrintView ? 'print:break-inside-avoid' : ''}`}>
            {[...Array(6)].map((_, i) => (
              <div key={i} className={`w-8 h-8 ${isPrintView ? 'bg-white' : 'bg-zinc-100'} border-t-2 border-slate-200 rounded-full`} />
            ))}
          </div>

...
export default function OnboardingClient({
  ...
  const handleRejectPaperwork = () => {
    if (!selectedActivation) return;
    startTransition(async () => {
      const result = await rejectTenantPaperwork(selectedActivation.id, selectedActivation.user.id);
      ...
      <div className="min-h-screen bg-zinc-100 font-sans print:bg-white">
        ...
        <div className="print-hide sticky top-0 z-10 flex items-center justify-between p-4 bg-white border-b border-zinc-200 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            >
              <HugeiconsIcon icon={ArrowLeft01Icon} size={18} className="mr-2" /> Back to Dashboard
            </Button>
            <div className="h-4 w-px bg-zinc-200" />
            <h3 className="font-bold text-zinc-800 flex items-center gap-2">
              <HugeiconsIcon icon={File02Icon} size={18}/> Tenancy Agreement
            </h3>
          </div>
          ...
          {selectedActivation && (
            <>
              ...
              <div className=" w-full p-4 border-t border-zinc-200/60 bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.02)]">
                ...
                >
                  <HugeiconsIcon icon={FileDownloadIcon} size={16} className={`mr-2 ${selectedActivation.checklist.leaseSigned !== "Signed" ? "text-zinc-300" : "text-zinc-500"}`} />

...
export default function OnboardingClient({
  ...
  // State for the split view & confirmations
  ...
  const filteredData = useMemo(() => {
    return data.filter((record) => {
      ...
      const matchesProperty =
        propertyFilter === "all" || record.lease.propertyName === propertyFilter;
  ...
  const executeConfirmedAction = (e: React.MouseEvent) => {
    ...
    startTransition(async () => {
      ...
      if (confirmAction === "approve") {
        result = await approveTenantPaperwork(selectedActivation.id, selectedActivation.user.id);
      } else if (confirmAction === "reject") {
        result = await rejectTenantPaperwork(selectedActivation.id, selectedActivation.user.id);
      ...
      setConfirmAction(null); // Close the dialog after completion
    });
  };

  // Helper object to dynamically populate the AlertDialog based on the current action
  const dialogContent = {
    approve: {
      title: "Approve Paperwork",
      description: "Are you sure you want to approve this tenant's ID and lease agreement? This verifies their identity and clears them for access.",
      confirmText: "Yes, Approve",
      confirmClass: "bg-black text-white hover:bg-zinc-800",
    },
    reject: {
      title: "Reject Paperwork",
      description: "Are you sure you want to reject these documents? The tenant will be notified immediately to submit new documentation.",
      confirmText: "Yes, Reject",
      confirmClass: "bg-rose-600 text-white hover:bg-rose-700",
    },
    pin: {
      title: "Generate Smart Lock PIN",
      description: "This will provision the Tuya smart lock and send the unique access PIN directly to the tenant's phone. Are you sure you want to grant physical access?",
      confirmText: "Generate & Grant Access",
      confirmClass: "bg-black text-white hover:bg-zinc-800",
    },
  };
      ...
      <div className="min-h-screen bg-zinc-100 font-sans print:bg-white">
        ...
        <div className="print-hide sticky top-0 z-10 flex items-center justify-between p-4 bg-white border-b border-zinc-200 shadow-sm">
          <div className="flex items-center gap-4">
            <Button variant="ghost" onClick={() => setIsViewingDocument(false)} className="text-zinc-600 hover:text-zinc-900">
              <HugeiconsIcon icon={ArrowLeft01Icon} size={18} className="mr-2" /> Back to Onboarding
            </Button>
            <div className="h-4 w-px bg-zinc-200" />
            <h3 className="font-bold text-zinc-800 flex items-center gap-2">Tenancy Agreement</h3>
          </div>
          <Button onClick={() => window.print()} className="bg-zinc-900 text-white hover:bg-zinc-800 h-9 px-6 text-xs font-bold uppercase tracking-wider rounded-lg shadow-sm">
            <HugeiconsIcon icon={PrinterIcon} size={16} className="mr-2" /> Print Document
          </Button>
        </div>
          ...
          <div className="p-8 md:p-12 font-serif text-zinc-900 leading-relaxed text-sm text-justify bg-white border border-zinc-200 rounded-2xl flex flex-col print:border-none print:shadow-none print:w-full print:block print:p-0 print:text-black print:text-sm">
            <div className="text-center mb-10">
              ...
              <p className="mt-4 font-sans text-[10px] uppercase tracking-widest text-zinc-500 font-bold">
                Reference ID: {selectedActivation.lease.id.slice(-12).toUpperCase()}
              </p>
            </div>

            <div className="space-y-6">
              <p>This Tenancy Agreement is formally established between <strong>WunkatHomes Ltd.</strong> (referred to as the "Landlord") and <strong>{selectedActivation.user.name}</strong> (referred to as the "Tenant").</p>
              <p><strong>1. The Property:</strong> The Landlord agrees to rent, and the Tenant agrees to occupy the property known as <strong>{selectedActivation.lease.propertyName}</strong> located at <strong>{selectedActivation.lease.propertyLocation || `Unit ${selectedActivation.lease.unitNumber}`}</strong>.</p>
              <p><strong>2. Lease Duration:</strong> This agreement begins on <strong>{formatDate(selectedActivation.lease.startDate)}</strong> and will remain active until <strong>{selectedActivation.lease.endDate ? formatDate(selectedActivation.lease.endDate) : "the end of the agreed term"}</strong>, unless ended earlier under the terms of this agreement.</p>
              <p><strong>3. Rent & Payment:</strong> The total rent payment of <strong>GHS {selectedActivation.lease.totalRentAmount?.toLocaleString(undefined, { minimumFractionDigits: 2 })}</strong> has been successfully processed and verified.</p>
              <p><strong>4. Smart Lock & Property Access:</strong> Access to the property is managed securely via a Tuya Smart Lock system. The Tenant agrees to keep their personal access PIN confidential and not share it with unauthorized individuals.</p>
              <p><strong>5. Tenant Responsibilities:</strong> The Tenant agrees to maintain the interior of the property in good condition, use the property only for residential living, and allow the Landlord or maintenance teams to enter for repairs with fair prior notice.</p>
            </div>
            ...
            <div className="mt-16 pt-8 border-t border-zinc-200 print:break-inside-avoid">
              ...
              <div className="grid grid-cols-2 gap-8 font-sans text-xs">
                ...
                <div>
                  ...
                  <p className="font-signature text-2xl font-bold">{selectedActivation.lease.signatureAudit?.typedName || selectedActivation.user.name}</p>
                  <p className="text-zinc-500 mt-2">IP Address: {selectedActivation.lease.signatureAudit?.ipAddress || "N/A"}</p>
                  <p className="text-zinc-500">Date: {selectedActivation.lease.signatureAudit?.signedAt || "Pending"}</p>
                </div>
              </div>
              <div className="mt-8 bg-zinc-50 p-4 rounded font-mono text-[10px] text-zinc-500 break-all border border-zinc-100 print:border-none print:bg-white print:p-0 print:text-black">
                <strong>Security ID:</strong> {selectedActivation.lease.signatureAudit?.documentHash || "Pending Generation"}
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] p-6 lg:pb-10 font-sans">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-200/60 pb-4">
          <div className="flex items-center gap-3">
            ...
            {awaitingCount > 0 && (
              <Badge variant="secondary" className="bg-black text-white hover:bg-zinc-800 text-[11px] px-2 h-5 flex items-center justify-center rounded-full">
                {awaitingCount} New
              </Badge>
            )}
          </div>
          ...
          <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as PipelineStage)} className="w-full md:w-auto">
            <TabsList className="h-9 bg-zinc-100/50 border border-zinc-200/60 p-0.5 rounded-lg">
              <TabsTrigger value="awaiting_paperwork" className="text-[13px] font-medium data-[state=active]:bg-white rounded-sm px-4">
                Awaiting Paperwork
              </TabsTrigger>
              <TabsTrigger value="ready_for_access" className="text-[13px] font-medium data-[state=active]:bg-white rounded-sm px-4">
                Ready for Access
              </TabsTrigger>
              <TabsTrigger value="recent" className="text-[13px] font-medium data-[state=active]:bg-white rounded-sm px-4">
                Recent Activations
              </TabsTrigger>
            </TabsList>
          </Tabs>
            ...
            <TableBody>
              {filteredData.map((record) => (
                ...
                >
                  ...
                  <TableCell className="py-3 align-middle">
                    <span className="text-[13px] font-medium text-zinc-700">{formatDate(record.lease.startDate)}</span>
                  </TableCell>
                  ...
                  <TableCell className="py-3 align-middle text-right">
                    ...
                    {activeTab === "ready_for_access" && (
                      <Button size="sm" className="h-8 text-[11px] font-semibold text-white bg-primary hover:bg-zinc-800 rounded-lg">
                        Activate Key
                      </Button>
                    )}
                    {activeTab === "recent" && (
      ...
      {expandedImage && (
        ...
        >
          <div className="relative max-w-4xl w-full h-full max-h-[85vh] flex flex-col items-center justify-center" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-4 right-4 text-white bg-black/50 hover:bg-black/80 rounded-full p-2 transition-colors z-10" onClick={() => setExpandedImage(null)}>
              <HugeiconsIcon icon={Cancel01Icon} size={24} />
            </button>
            <img src={expandedImage} alt="Expanded View" className="max-w-full max-h-full object-contain rounded-lg shadow-2xl relative z-0" />
          </div>
        </div>
      )}
            ...
            <>
              ...
              <div className="px-6 pt-10 pb-6 border-b border-zinc-200/60 bg-white">
                <div className="flex items-center justify-between mb-5">
                  <Badge variant="outline" className={`px-2.5 py-0.5 border-0 rounded-sm text-[10px] uppercase tracking-wider font-bold ${selectedActivation.pipelineStage === "ready_for_access" ? "bg-indigo-50 text-indigo-700 ring-1 ring-indigo-200/60" : selectedActivation.pipelineStage === "recent" ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200/60" : "bg-amber-50 text-amber-700 ring-1 ring-amber-300/60"}`}>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-semibold tracking-tight text-zinc-900 leading-tight">{selectedActivation.user.name}</h2>
                    <p className="text-[12px] text-zinc-500 mt-0.5 flex items-center gap-1.5">{selectedActivation.user.email}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <section className="bg-white border border-zinc-200/60 rounded-xl p-5 shadow-[0_1px_4px_rgba(0,0,0,0.01)] space-y-5">
                  <div className="border border-zinc-200/80 rounded-lg overflow-hidden group">
                    <div className="h-24 w-full bg-zinc-100 relative cursor-pointer flex items-center justify-center" onClick={() => selectedActivation.user.ghanaCardUrl && setExpandedImage(selectedActivation.user.ghanaCardUrl)}>
                      {selectedActivation.user.ghanaCardUrl ? (
                        <>
                          <img src={selectedActivation.user.ghanaCardUrl} alt="Ghana Card" className="w-full h-full object-cover mix-blend-multiply opacity-80 group-hover:scale-105 transition-transform duration-500" />
                          <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/10 transition-colors">
                            <span className="opacity-0 group-hover:opacity-100 bg-white/90 backdrop-blur-sm text-zinc-900 text-[10px] uppercase font-bold tracking-wider px-3 py-1.5 rounded-full shadow-sm transition-opacity">View Full Size</span>
                          </div>
                        </>
                      ) : (
                  ...
                  {!isLegalApproved && (
                    <div className="flex gap-3">
                      <Button className="flex-1 h-9 bg-black text-white rounded-lg hover:bg-zinc-800 text-[12px] font-semibold" onClick={() => setConfirmAction("approve")}>
                        Approve
                      </Button>
                      <Button variant="outline" className="flex-1 h-9 border-rose-200 text-rose-700 bg-rose-50 hover:bg-rose-100 hover:text-rose-800 rounded-lg text-[12px] font-semibold" onClick={() => setConfirmAction("reject")}>
                        Reject
                      </Button>
                    </div>
                  )}
                ...
                <section className={`bg-white border border-zinc-200/60 rounded-xl p-5 shadow-[0_1px_4px_rgba(0,0,0,0.01)] space-y-4 transition-all duration-300 ${!isLegalApproved ? "opacity-50 grayscale pointer-events-none" : ""}`}>
                  ...
                  {selectedActivation.smartLockPin ? (
                    ...
                    <Button className="w-full h-10 mt-2 rounded-lg bg-black text-white hover:bg-zinc-800 text-[13px] font-semibold shadow-sm" onClick={() => setConfirmAction("pin")}>
                      <HugeiconsIcon icon={Key01Icon} size={14} className="mr-2" /> Generate & Sync Smart Lock PIN
                    </Button>
                  )}
                </section>
                ...
                {!isLegalApproved && (
                  <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200/50">
                    ...
                    <p className="text-[11px] font-medium text-amber-800 leading-tight">Smart lock provisioning is locked until all identity documents and lease agreements are verified and approved.</p>
                  </div>
                )}
              </div>
              ...
              <div className="w-full p-4 border-t border-zinc-200/60 bg-white shadow-[0_-4px_12px_rgba(0,0,0,0.02)]">
                <Button variant="outline" className="w-full rounded-lg h-10 border-zinc-200 hover:bg-zinc-50 text-zinc-900 font-medium disabled:bg-zinc-50 disabled:text-zinc-400" onClick={() => setIsViewingDocument(true)} disabled={selectedActivation.checklist.leaseSigned !== "Signed"}>
                  <HugeiconsIcon icon={FileDownloadIcon} size={16} className={`mr-2 ${selectedActivation.checklist.leaseSigned !== "Signed" ? "text-zinc-300" : "text-zinc-500"}`} /> View Tenancy Agreement
                </Button>
              </div>
            </>
          )}
          ...
          {confirmAction && (
            <>
              ...
              <AlertDialogFooter className="mt-6 gap-2 sm:gap-0">
                <AlertDialogCancel disabled={isPending} className="h-10 text-[13px] font-semibold border-zinc-200 hover:bg-zinc-50 rounded-xl">
                  Cancel
                </AlertDialogCancel>
                <AlertDialogAction 
                ...
                >
                  {isPending ? (
                    <><HugeiconsIcon icon={Loading03Icon} className="animate-spin mr-2" size={14} /> Processing...</>
                  ) : (

...
const exploreData = {
  rent: {
    ...
    description: "Subscription-based luxury. Fully managed, turnkey spaces with instant Tuya smart-lock access.",
    collections: [
      { title: "Smart Studios", image: "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?q=80&w=2070", link: "/properties?type=apartment&status=For_Rent" },
      { title: "Corporate Suites", image: "https://images.unsplash.com/photo-1497366216548-37526070297c?q=80&w=2069", link: "/properties?type=apartment&status=For_Rent" },
      { title: "Suburban Retreats", image: "https://images.unsplash.com/photo-1583608205776-bfd35f0d9f83?q=80&w=2070", link: "/properties?type=house&status=For_Rent" },
    ]
  },
  sale: {
    heroTitle: "Acquire",
    ...
    description: "100% verified portfolio assets. Seamless digital acquisition and immediate ownership transfer.",
    collections: [
      { title: "Penthouses", image: "https://images.unsplash.com/photo-1600607688969-a5bfcd64bd2b?q=80&w=2070", link: "/properties?type=apartment&status=For_Sale" },
      { title: "Commercial Hubs", image: "https://images.unsplash.com/photo-1497366754035-f200968a6e72?q=80&w=2069", link: "/properties?type=commercial&status=For_Sale" },
      { title: "Prime Land Plots", image: "https://images.unsplash.com/photo-1500382017468-9049fed747ef?q=80&w=2032", link: "/properties?type=land&status=For_Sale" },
    ]
  }
}
      ...
      <section className="relative h-[80vh] md:h-[90vh] w-full flex items-end pb-12 md:pb-24">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex flex-col md:flex-row justify-between items-end gap-8">
          ...
          >
            ...
            <Link href={`/properties?status=For_${mode.charAt(0).toUpperCase() + mode.slice(1)}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 h-auto md:h-[600px]">
          ...
          >
            <Link href={content.collections[0].link}>
              <Image src={content.collections[0].image} alt={content.collections[0].title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
              <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors" />
              <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end">
                <h3 className="text-3xl md:text-4xl font-black text-white uppercase">{content.collections[0].title}</h3>
          ...
          <div className="md:col-span-5 flex flex-col gap-6">
            ...
            >
              <Link href={content.collections[1].link}>
                <Image src={content.collections[1].image} alt={content.collections[1].title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors" />
                <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                  <h3 className="text-2xl font-black text-white uppercase max-w-[150px] leading-tight">{content.collections[1].title}</h3>
            ...
            >
              <Link href={content.collections[2].link}>
                <Image src={content.collections[2].image} alt={content.collections[2].title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute inset-0 bg-black/30 group-hover:bg-black/40 transition-colors" />
                <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                  <h3 className="text-2xl font-black text-white uppercase max-w-[150px] leading-tight">{content.collections[2].title}</h3>
                  <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-black -rotate-45 group-hover:rotate-0 transition-transform duration-300">
                    <HugeiconsIcon icon={ArrowRight01Icon} size={20} />
                  </div>
                </div>
...
export default function ExplorePage() {
  return (
    <Suspense fallback={<div className="h-screen w-full bg-black flex items-center justify-center text-white font-bold uppercase tracking-widest text-sm animate-pulse">Initializing Protocol...</div>}>
      <ExploreView />
    </Suspense>
  )
}
...
export default function HelpAndSupportPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 pb-24">
      ...
      {/* --- HERO & SEARCH SECTION --- */}
      <section className="bg-white border-b border-zinc-200/60 pt-16 pb-20 px-4 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center">
          ...
          <form 
            ...
            className="relative max-w-2xl mx-auto flex items-center bg-white border border-zinc-200/60 rounded-lg shadow-sm p-1.5 focus-within:ring-2 focus-within:ring-zinc-950/20 focus-within:border-zinc-950 transition-all"
          >
            ...
            <input 
              ...
              className="w-full py-3 bg-transparent text-zinc-900 placeholder:text-zinc-400 focus:outline-none text-[15px]"
            />
            <button 
              type="submit"
              className="bg-zinc-950 hover:bg-zinc-800 text-white px-6 py-3 rounded-lg font-medium transition-colors shrink-0"
            >
              Search
            </button>
          </form>
          
          <p className="text-xs text-zinc-400 mt-6 uppercase tracking-wider font-semibold">
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-8 relative z-10">
          ...
          {supportCategories.map((category) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center text-center p-6 rounded-lg bg-white border transition-all duration-200 ${
                  isActive 
                    ? "border-zinc-950 shadow-sm ring-1 ring-zinc-950 scale-105" 
                    : "border-zinc-200/60 shadow-sm hover:border-slate-300 hover:shadow-sm hover:-translate-y-1"
                }`}
              >
                <div className={`p-3 rounded-full mb-4 transition-colors ${
                  isActive ? "bg-zinc-950 text-white" : "bg-zinc-100/50 text-zinc-600"
                }`}>
                  <HugeiconsIcon icon={category.icon} size={24} />
                </div>
                <h3 className={`font-semibold text-[15px] mb-2 ${isActive ? "text-zinc-950" : "text-zinc-800"}`}>
                  {category.title}
                </h3>
        ...
        <section className="bg-white rounded-lg border border-zinc-200/60 shadow-sm p-8 md:p-12 mb-16">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-bold text-zinc-900 mb-3">Frequently Asked Questions</h2>
            ...
            {faqs.map((faq, index) => {
              ...
              return (
                <div key={index} className="py-2">
                  ...
                  >
                    <span className={`font-medium pr-4 transition-colors ${isOpen ? "text-zinc-950" : "text-zinc-700 group-hover:text-zinc-950"}`}>
                      {faq.question}
                    </span>
                    <div className={`p-1 rounded-full border transition-all duration-300 shrink-0 flex items-center justify-center ${
                      isOpen ? "bg-zinc-950 border-zinc-950 text-white" : "bg-white border-zinc-200/60 text-zinc-400 group-hover:border-slate-300"
                    }`}>
                      <HugeiconsIcon 
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <p className="pb-6 text-[15px] leading-relaxed text-zinc-600 pr-12">
                          {faq.answer}
                        </p>
                      </motion.div>
                    )}
        ...
        <section className="text-center">
          <h2 className="text-2xl font-bold text-zinc-900 mb-3">You still have a question?</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
            ...
            {/* Phone Card */}
            <div className="bg-white border border-zinc-200/60 rounded-lg p-8 flex flex-col items-center justify-center hover:shadow-sm transition-shadow">
              <div className="h-12 w-12 rounded-full bg-zinc-100/50 flex items-center justify-center text-zinc-600 mb-4">
                <HugeiconsIcon icon={Call02Icon} size={24} />
              </div>
              <h3 className="font-bold text-zinc-900 text-lg mb-1">+ (233) 000-000-000</h3>
              <p className="text-sm text-zinc-500 mb-4">We are always happy to help.</p>
            ...
            <div className="bg-white border border-zinc-200/60 rounded-lg p-8 flex flex-col items-center justify-center hover:shadow-sm transition-shadow">
              ...
              <h3 className="font-bold text-zinc-900 text-lg mb-1">support@wunkathomes.com</h3>
              <p className="text-sm text-zinc-500 mb-4">The best way to get an answer faster.</p>
              <button className="text-sm font-semibold text-zinc-950 flex items-center gap-1 hover:underline underline-offset-4">

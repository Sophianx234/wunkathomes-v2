...
interface TransactionRecord {
  ...
  createdAt: string;
  amount: number;
  paymentPurpose: "Booking_Deposit" | "Rent_Balance" | "Monthly_Renewal" | "Purchase";
...
const MOCK_TRANSACTIONS: TransactionRecord[] = [
  {
    ...
    proofOfPaymentUrl: "https://images.unsplash.com/photo-1621360841013-c76831f1e35d?q=80&w=400&auto=format&fit=crop",
    user: {
      name: "Kwame Mensah",
...
const getStatusBadge = (status: TransactionRecord["status"]) => {
  switch(status) {
    case "Completed": return "text-emerald-700 ring-1 ring-emerald-200/60 bg-emerald-50/30";
    case "Pending_Verification": return "text-amber-700 ring-1 ring-amber-300/60 bg-amber-50/50 animate-pulse-slow";
    case "Failed": return "text-zinc-500 ring-1 ring-zinc-200/60 bg-zinc-50/50 line-through decoration-zinc-300";
    case "Refunded": return "text-rose-700 ring-1 ring-rose-200/60 bg-rose-50/50";
  }
};

// --- MAIN LEDGER COMPONENT ---
export default function TransactionsLedgerPage() {
  const [activeTab, setActiveTab] = useState<"needs-review" | "history">("needs-review");
  ...
  const pendingCount = MOCK_TRANSACTIONS.filter(t => t.status === "Pending_Verification").length;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-200/60 pb-4">
          ...
          <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as any)} className="w-full md:w-auto">
            <TabsList className="h-9 bg-zinc-100/50 border border-zinc-200/60 p-0.5 rounded-lg">
              <TabsTrigger value="needs-review" className="text-[13px] font-medium data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-md px-4">
                Needs Review
                {pendingCount > 0 && (
              ...
              <TabsTrigger value="history" className="text-[13px] font-medium data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-md px-4">
                Transaction History
              </TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <Table>
            <TableHeader className="bg-zinc-50/30">
              <TableRow className="border-zinc-200/60 hover:bg-transparent">
                ...
                <TableHead className="font-medium text-zinc-500 text-xs h-10 text-right">Value</TableHead>
                <TableHead className="font-medium text-zinc-500 text-xs h-10 w-[140px] text-right">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredData.map((tx) => (
                ...
                >
                  ...
                  <TableCell className="py-3 align-middle">
                    <div className="flex flex-col">
                      <span className="text-[13px] font-mono font-medium text-zinc-900">{tx.transactionReference}</span>
                      <span className="text-[11px] text-zinc-500 mt-0.5">{formatDate(tx.createdAt)}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="py-3 align-middle">
                    <div className="flex items-center gap-2.5">
                      <Avatar className="h-7 w-7 border border-zinc-200/60 shadow-sm">
                        ...
                        <AvatarFallback className="bg-zinc-100 text-zinc-600 text-[10px]">{tx.user.name.charAt(0)}</AvatarFallback>
                      </Avatar>
                      <span className="text-[13px] font-medium text-zinc-900">{tx.user.name}</span>
                    </div>
                  </TableCell>

                  {/* Col 3: Allocation */}
                  <TableCell className="py-3 align-middle">
                    <span className={`px-2 py-0.5 rounded-md text-[11px] font-medium tracking-wide ${getPurposeBadge(tx.paymentPurpose)}`}>
                  ...
                  <TableCell className="py-3 align-middle">
                    <div className="flex items-center text-[12px] font-medium text-zinc-600 gap-1.5">
                      {tx.paymentMethod === "Paystack" ? (
                        <>
                          Paystack <HugeiconsIcon icon={ArrowUpRight01Icon} size={12} className="text-zinc-400" />
                        </>
                      ) : (
                        <>
                          <HugeiconsIcon icon={UniversityIcon} size={12} className="text-zinc-400" /> Bank Transfer
                        </>
                      )}
                    </div>
                  </TableCell>
                  ...
                  {/* Col 6: Status */}
                  <TableCell className="py-3 align-middle text-right">
                    <Badge variant="outline" className={`px-2 py-0 border-0 rounded-full text-[10px] uppercase tracking-wider font-bold h-5 ${getStatusBadge(tx.status)}`}>
                      {tx.status.replace("_", " ")}
                    </Badge>
                  </TableCell>
                </TableRow>
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto pb-32">
                ...
                <div className="px-6 pt-10 pb-6 border-b border-zinc-100 bg-zinc-50/30">
                  <Badge variant="outline" className={`mb-4 px-2 py-0 border-0 rounded-full text-[10px] uppercase tracking-wider font-bold h-5 ${getStatusBadge(selectedTx.status)}`}>
                  ...
                  <div className="bg-white border border-zinc-200/60 rounded-lg p-3 flex items-center gap-3 ">
                    <Avatar className="h-10 w-10 border border-zinc-200/60">
                      ...
                      <AvatarFallback className="bg-zinc-100 text-zinc-600 text-xs">{selectedTx.user.name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="text-[13px] font-medium text-zinc-900 leading-tight">{selectedTx.user.name}</p>
                      <p className="text-[11px] text-zinc-500 mt-0.5">{selectedTx.user.email}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-6 py-6 space-y-8">
                  ...
                  {/* Audit Details Block */}
                  <section>
                    <h3 className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Audit Log</h3>
                    <div className="bg-white border border-zinc-200/60 rounded-xl overflow-hidden text-[13px]">
                      ...
                      <div className="flex justify-between py-2.5 px-4 border-b border-zinc-100">
                        ...
                        <span className="text-zinc-900 font-mono tracking-tight">{selectedTx.transactionReference}</span>
                      </div>
                      
                      <div className="flex justify-between py-2.5 px-4 border-b border-zinc-100">
                        <span className="text-zinc-500 font-medium">Timestamp</span>
                        <span className="text-zinc-900 font-tabular-nums">{formatDate(selectedTx.createdAt)}</span>
                      </div>
                      
                      <div className="flex justify-between py-2.5 px-4 border-b border-zinc-100">
                        <span className="text-zinc-500 font-medium">Lease Link</span>
                        <div className="flex items-center gap-1 text-zinc-900">
                          <HugeiconsIcon icon={LinkSquare01Icon} size={14} className="text-indigo-500" />
                          <span className="font-mono tracking-tight">{selectedTx.lease.id}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex justify-between py-2.5 px-4 bg-zinc-50/50">
                        ...
                        <span className="text-zinc-900 text-right">
                          ...
                          <span className="text-[11px] text-zinc-500">{selectedTx.lease.unitNumber}</span>
                        </span>
                      </div>
                    </div>
                  </section>
                  ...
                  {selectedTx.paymentMethod === "Bank_Transfer" && (
                    <section>
                      <h3 className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Proof of Payment</h3>
                      ...
                      {selectedTx.proofOfPaymentUrl ? (
                        <div className="relative group rounded-xl overflow-hidden border border-zinc-200/80 bg-zinc-100 cursor-pointer">
                          {/* Top Bar Chrome */}
                          <div className="absolute top-0 w-full h-8 bg-black/40 backdrop-blur-md flex items-center px-3 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                            <HugeiconsIcon icon={File02Icon} size={14} className="text-white/80 mr-2" />
                            <span className="text-[10px] text-white/90 font-mono uppercase tracking-wider">receipt_scan.jpg</span>
                          </div>
                          ...
                          <div className="absolute inset-0 bg-zinc-900/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <div className="bg-white/90 backdrop-blur-sm text-zinc-900 text-xs font-semibold px-4 py-2 rounded-full shadow-lg flex items-center gap-2">
                              <HugeiconsIcon icon={SearchMinusIcon} size={14} /> Enlarge Document
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="border border-dashed border-zinc-300 rounded-xl p-6 flex flex-col items-center justify-center text-zinc-400 bg-zinc-50">
                          <HugeiconsIcon icon={Alert01Icon} size={24} className="mb-2 opacity-50" />
                          <p className="text-[12px] font-medium text-center">No document uploaded.</p>
                        </div>
                      )}
                    </section>
                  )}
                ...
                <div className="absolute bottom-0 left-0 w-full bg-white border-t border-zinc-200/80 p-4 space-y-3 shadow-[0_-8px_20px_rgba(0,0,0,0.03)] z-20">
                  ...
                  {showRejectNote && (
                    <div className="animate-in slide-in-from-bottom-2 fade-in duration-200 mb-2">
                      <label className="text-[11px] font-medium text-zinc-500 uppercase tracking-wide mb-1 block">Rejection Reason</label>
                      <Textarea 
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button 
                      className={`flex-1 h-10 font-semibold rounded-lg transition-all duration-200 ${showRejectNote ? 'bg-zinc-100 text-zinc-400 hover:bg-zinc-200 pointer-events-none' : 'bg-zinc-900 text-white hover:bg-zinc-800 shadow-sm'}`}
                    >

...
async function fetchSpecs() {
  ...
  const path2 = `/v1.0/devices/${deviceId}/specifications`;
  
  const { sign: sign2, t: t2 } = calculateSignature('GET', path2, '', token);
  const res2 = await fetch(`${TUYA_ENDPOINT}${path2}`, {
    headers: { client_id: TUYA_ACCESS_ID, sign: sign2, t: t2, sign_method: 'HMAC-SHA256', access_token: token }
  });
